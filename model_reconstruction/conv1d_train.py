...
from window_layers import IDCNN
...
def build_model(transform, embeddings=100, embedding_trainable=False,rnn_input_dropout=0.2,
    ...
    model.add(IDCNN(repeat_times=2, dilations=[1,1,2],filters=[256,256,512],kernel_size=[2,3,4]))
...
def build(transform):
    return_dict=build_model(transform,embeddings=100, embedding_trainable=True,
            rnn_input_dropout=0.5, rnn_output_dropout=0.5, rnn_units=200,crf_lr_multiplier=1e-2)
    model,CRF = return_dict['model'],return_dict['CRF']
...
#     tf.print(f'dev_steps: {dev_steps}')
    ...
    checkpoint = tf.keras.callbacks.ModelCheckpoint(
                os.path.join(save_model, '0128_model_conv1d.h5'),
                # verbose=1,
...
#     early_stopping = tf.keras.callbacks.EarlyStopping(monitor=monitor, patience=15)
    tensorboard_callback = tf.keras.callbacks.TensorBoard(
        log_dir=io_util.makedirs(io_util.path_join(save_model, 'logs_conv1d')))
