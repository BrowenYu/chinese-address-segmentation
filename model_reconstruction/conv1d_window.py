...
from window_layers import IDCNN,WindowEmbedding
...
trn_path = PATH+'generate/train0121_all.txt'
dev_path = PATH+'generate/dev0121_all.txt'
...
# @tf.function
def build_model(transform, embeddings=100, embedding_trainable=False,window_size=1,rnn_input_dropout=0.2,
...
#                                             trainable=True, mask_zero=True)
    output=WindowEmbedding(window_size, input_dim=len(transform.word_vocab), output_dim=embeddings,
                                        trainable=True, mask_zero=True)(S_inputs)
    if rnn_input_dropout:
        output=tf.keras.layers.Dropout(rnn_input_dropout, name='rnn_input_dropout')(output)
#     output=tf.keras.layers.Bidirectional(tf.keras.layers.LSTM(units=rnn_units, return_sequences=True), name='bilstm')(output)
    output=IDCNN(repeat_times=4, dilations=[1,1,2],filters=[256,256,512],kernel_size=[2,3,4])(output)
    
    if rnn_output_dropout:
        output=tf.keras.layers.Dropout(rnn_output_dropout, name='rnn_output_dropout')(output)
    output=tf.keras.layers.TimeDistributed(tf.keras.layers.Dense(len(transform.tag_vocab)), name='dense')(output)
...
def build(transform):
    model,CRF=build_model(transform,embeddings=100, embedding_trainable=True,window_size=3,
            rnn_input_dropout=0.5, rnn_output_dropout=0.5, rnn_units=200,crf_lr_multiplier=1e-2)
...
#     tf.print(f'dev_steps: {dev_steps}')
    ...
    checkpoint = tf.keras.callbacks.ModelCheckpoint(
                os.path.join(save_model, '0201_idcnn_window.h5'),
                # verbose=1,
    ...
    tensorboard_callback = tf.keras.callbacks.TensorBoard(
        log_dir=io_util.makedirs(io_util.path_join(save_model, 'logs_idcnn_window')))
