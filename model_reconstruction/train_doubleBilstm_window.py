...
def build_model(embeddings=100,vocab_size=vocab_size,rnn_units=100,window_size=1):
   
    x_in = Input(shape=(None,))
    
    output = WindowEmbedding(window_size, input_dim=vocab_size, output_dim=embeddings,
                                        trainable=True, mask_zero=True)(x_in)
#     output=Embedding(input_dim=vocab_size, output_dim=embeddings,
#                                             trainable=True, mask_zero=True)(x_in)
    
    
    output=Bidirectional(LSTM(units=rnn_units, return_sequences=True),name='bilstm_1')(output)
    
    seq_output=Dropout(0.5)(output)
    
    
    output=Bidirectional(LSTM(units=rnn_units, return_sequences=True),name='bilstm_2')(output)
...
class Evaluator(keras.callbacks.Callback):
    ...
    def on_epoch_end(self, epoch, logs=None):
        ...
        if seq_acc >= self.best_val_seq:
            ...
            self.model.save_weights('./model/best_seq_double_0118.h5')
        if tag_acc >= self.best_val_tag:
            self.best_val_tag = tag_acc
            self.model.save_weights('./model/best_tag_double_0118.h5')
        print(
...
valid_data=get_valid(train_file[-20000:])
# print(valid_data[:10])
...
model,Seq_crf,Tag_crf = build_model(embeddings=200,vocab_size=vocab_size,rnn_units=300,window_size=4)
...
checkpoint = ModelCheckpoint('./model/best_0118_window.hdf5', monitor='val_tag_crf_Sparse_accuracy', verbose=2, save_best_only=True, mode='max',
