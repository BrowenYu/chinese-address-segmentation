...
def build_model(transform, embeddings=100, embedding_trainable=False,rnn_input_dropout=0.2,
    ...
    model.add(tf.keras.layers.Bidirectional(tf.keras.layers.LSTM(units=200, return_sequences=True), name='bilstm_1'))
    model.add(tf.keras.layers.Conv1D(filters =800,kernel_size = 5,padding ='same',activation ='relu'))
...
def build(transform):
    model,CRF = build_model(transform,embeddings=200, embedding_trainable=True,
...
#     tf.print(f'dev_steps: {dev_steps}')
    ...
    checkpoint = tf.keras.callbacks.ModelCheckpoint(
                os.path.join(save_model, '0128_bilstm_conv1d.h5'),
                # verbose=1,
                monitor=monitor, save_best_only=True,
                mode='max',
                save_weights_only=True)
    
    early_stopping = tf.keras.callbacks.EarlyStopping(monitor=monitor, patience=20)
    tensorboard_callback = tf.keras.callbacks.TensorBoard(
        log_dir=io_util.makedirs(io_util.path_join(save_model, 'logs_0128')))
